...
// `from`/`to` are inclusive ISO date strings (YYYY-MM-DD). When given, the
// range is filtered server-side so a wide custom range isn't silently capped
// by the default 200-row window. Omit both for the latest 200 (dashboard).
export function useEntries({ from, to } = {}) {
